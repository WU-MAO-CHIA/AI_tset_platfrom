...
| content | TEXT | NOT NULL | 訊息內容（assistant 訊息包含結構化分隔符） |
| created_at | DATETIME | NOT NULL | 建立時間 |

**Content 格式（assistant 訊息）**:
```
{人類可讀的AI回應文字}---RF_CODE---
*** Settings ***
...Robot Framework 腳本...
...
**Validation Rules**:
- `role` 限 `user` 或 `assistant`
- 前端解析：顯示 `---RF_CODE---` 之前的部分作為對話氣泡；RF 程式碼預覽從 `---RF_CODE---` 至 `---END---` 之間提取
- 頁面重載時從最後一條 `role=assistant` 訊息恢復 RF code（`onMounted` 邏輯）
