...
- [X] T259 [P] 在 `frontend/tests/e2e/trial-run.spec.ts` 新增 e2e 測試：（1）開啟案例編輯頁面進入 Tab 2；（2）輸入簡單 RF 代碼於右側預覽區或由 AI Chat 生成；（3）驗證「立即試跑」按鈕啟用；（4）清空 RF 代碼區，驗證「立即試跑」按鈕禁用且提示『RF 程式碼為空，請先透過 Chat 生成程式碼』；（5）恢復 RF 代碼，點擊「立即試跑」按鈕；（6）驗證試跑結果訊息出現在左側 Chat 區域；（7）若試跑失敗，驗證 AI 建議訊息隨後出現；（8）刷新頁面後驗證訊息持久化至對話歷史
